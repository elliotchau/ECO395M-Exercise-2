...
```{r confusion_out2, echo=FALSE}
confusion_out2
```
```{r error rate, echo=FALSE}
1-sum(diag(confusion_out2))/sum(confusion_out2)
```
```{r true positive, echo=TRUE}
807/(807+3074) #(1,1)/[(1,1)+(0,1)]
```
```{r false positive, echo=TRUE}
330/(3718+330) #(1,0)/[(1,0)+(0,0)]
```
```{r compare against null, echo=FALSE}
...
```{r confusion_out2 2, echo=FALSE}
confusion_out2
```
```{r error rate 2, echo=FALSE}
1-sum(diag(confusion_out2))/sum(confusion_out2)
```
```{r true positive 2, echo=TRUE}
834/(834+3026) #(1,1)/[(1,1)+(0,1)]
```
```{r false positive 2, echo=TRUE}
344/(344+3725) #(1,0)/[(1,0)+(0,0)]
```
```{r compare against null 2, echo=FALSE}
length(which(online_news$viral == 0))/length(online_news$viral)
```
